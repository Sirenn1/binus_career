...
export function SideMenuAdmin({
  ...
  const findCollapse = (e: string) => {
    if (openCollapse?.find((collapse) => collapse === e)) return true;
    return false;
  };

  const getSideMenu = async () => {
    const sideMenuItems = await apiClient.get<SideMenuResponse>(ApiService.getSideMenu);
